...
def IrisURLNormalization(path):
  ...
  #פונקציה למציאת קצוות בתמונה
  #1, 0 X מפעיל את הפילטר רק בציר 
  #0, 1 Y מפעיל את הפילטר רק בציר
  #ksize=3 - 3x3 גודל הפילטר
  #cv2.CV_64F - כך שהמספרים מדוייקים יותר כי הם לא מעוגלים float64מחשב ב
  sobel_x = cv2.Sobel(img, cv2.CV_64F, 1, 0, ksize=3)
  sobel_y = cv2.Sobel(img, cv2.CV_64F, 0, 1, ksize=3)

  #חישוב חוזקת הגראדיאנט שזה בעצם עוצמת הקצוות בכל פיקסל בתמונה
  #החישוב הוא לפי נוסחה של שורש ריבועי של סכום ריבועי של שני הפילטרים
  sobel_combined = np.sqrt(sobel_x**2 + sobel_y**2)
  #נורמליזציה של התמונה כך שהערכים יהיו בין 0 ל-255 מכיוון שההעלאה בריבוע עלולה ליצור ערכים גדולים מ255
  #np.unit8 - ממיר את התמונה למספרים שלמים כדי שנוכל להציג אותה אחר כך
