...
def preprocess(uploaded_file, segmentation, model):
    ...
    #אם לתמונה יש רק שני ממדים, כלומר שחור לבן אז תוסיף מימד שלישי בגודל 3 כדי שתוכל להיכנס למודל
    if img.ndim == 2:
        #np.stack - מחזיר מערך חדש עם ממד נוסף
        #axis=-1 - המימד החדש יתווסף בסוף כלומר אחרי המימד האחרון
        img = np.stack([img] * 3, axis=-1)
    #אם במקום, כן יש ממד שלישי אבל הוא שווה לאחד אז תהפוך אותו להיות שווה 3
    elif img.shape[-1] == 1:
        #np.concatenate - משכפל שלוש פעמים בכדי לגרום לממד השלישי להיות שווה 3
        #axis=-1 - המימד החדש יתווסף בסוף כלומר אחרי המימד האחרון
...
if img_file1 and img_file2:
    ...
    if img1 is None or img2 is None:
        ...
        #img.squeeze() / 255.0 - ממיר את התמונה למערך של פיקסלים בין 0 ל-1 
